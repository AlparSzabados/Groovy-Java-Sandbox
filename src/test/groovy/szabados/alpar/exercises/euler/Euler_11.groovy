...
class Euler_11 {
    ...
    public void shouldSolveProblem11() {
        final GRID = [[8, 2, 22, 97, 38, 15, 0, 40, 0, 75, 4, 5, 7, 78, 52, 12, 50, 77, 91, 8],
        ...
        assert greatestProductOfAdjacentNumbersInAGrid(GRID, 4) == 70600674;
    }

    private static greatestProductOfAdjacentNumbersInAGrid(List<List<Integer>> grid, int sizeOfConsecutive) {
    ...
    private static rightAndDown(List<List<Integer>> grid, int sizeOfConsecutive) {
        def maxRightAndDown = 0
        for (int k in 0..<grid.size()) {
            for (int i in 0..(grid.size() - sizeOfConsecutive)) {
                def horizontal = 1
                def vertical = 1
    ...
    private static diagonalRight(List<List<Integer>> grid, int sizeOfConsecutive) {
        def maxDiagonalRight = 0
        for (int k in 0..(grid.size() - sizeOfConsecutive)) {
            for (int i in 0..(grid.size() - sizeOfConsecutive)) {
                def result = 1
                def x = 0
    ...
    private static diagonalLeft(List<List<Integer>> grid, int sizeOfConsecutive) {
        def maxDiagonalLeft = 0
        for (int k in 0..(grid.size() - sizeOfConsecutive)) {
            for (int i in (sizeOfConsecutive - 1)..<grid.size()) {
                def result = 1
                def x = 0
                for (int j = k; j < k + sizeOfConsecutive; j++) {
                    result *= grid[j][i + x]
